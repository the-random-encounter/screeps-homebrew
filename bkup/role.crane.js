        ...
        if (!creep.memory.disableAI) {
            ...
            if (creep.memory.atCraneSpot == true) {
                ...
                if (creep.memory.dropLink == true) {
                    ...
                    } else if (objDestination.store.getFreeCapacity() >= objLink.store.getUsedCapacity() && objLink.cooldown == 0) {
                        if (creep.store.getUsedCapacity() == 0) {
                            console.log('crane: getting energy for C2D xfer');
                            creep.withdraw(objStorage, RESOURCE_ENERGY);
                        } else {
                            console.log('crane: transferring energy to link for C2D xfer');
