...
Room.prototype.cacheObjects 							= function cacheObjects() {
  ...
  const links 			= _.filter(allStructures, { structureType: STRUCTURE_LINK				} );
	const extractors 	= _.filter(allStructures, { structureType: STRUCTURE_EXTRACTOR 	} );
...
Room.prototype.initRoomFlags 							= function initRoomFlags() {
	...
	if (this.memory.flags.centralStorageLogic 				=== undefined)
		this.memory.flags.centralStorageLogic 					= false;
	...
	if (this.memory.flags.upgradersSeekEnergy === undefined)
		this.memory.flags.upgradersSeekEnergy = false;

	return '[' + this.name + ']: Room flags initialized: craneUpgrades(' + this.memory.flags.craneUpgrades + ') centralStorageLogic(' + this.memory.flags.centralStorageLogic + ') repairRamparts(' + this.memory.flags.repairRamparts + ') repairWalls(' + this.memory.flags.repairWalls + ') runnersDoMinerals(' + this.memory.flags.runnersDoMinerals + ') towerRepairBasic(' + this.memory.flags.towerRepairBasic + ') towerRepairDefenses(' + this.memory.flags.towerRepairDefenses + ') runnersDoPiles(' + this.memory.flags.runnersDoPiles + ') harvestersFixAdjacent(' + this.memory.flags.harvestersFixAdjacent + ') repairBasics(' + this.memory.flags.repairBasics + ') upgradersSeekEnergy(' + this.memory.flags.upgradersSeekEnergy + ')';
}
Room.prototype.setRoomFlags 							= function setRoomFlags([flags]) {
	...
	if (flag4)
		this.memory.flags.centralStorageLogic 					= flag4;
	...
	if (flag11)
		this.memory.flags.upgradersSeekEnergy 	= flag11;
...
Room.prototype.registerLogisticalPairs = function registerLogisticalPairs() {
	...
	let mineralOutbox;
	if (minerals) {
		mineralOutbox = minerals[0].pos.findInRange(FIND_STRUCTURES, 5, { filter: { structureType: STRUCTURE_CONTAINER } });
	...
	if (this.controller.level > 3 && storage) {
		for (let i = 0; i < energyOutboxes.length; i++) {
			let onePair = [energyOutboxes[i], storage, 'energy', 'source to storage  '];
			logisticalPairs.push(onePair);
		}
		let onePair = [storage, energyInbox, 'energy', 'storage to upgrader'];
		logisticalPairs.push(onePair);
		if (mineralOutbox) {
			const mineral = Object.keys(Game.getObjectById(mineralOutbox).store);
			onePair = [mineralOutbox, storage, mineral[0], 'extractor to storage'];
			console.log(onePair);
			logisticalPairs.push(onePair);
		}
	} else {
		for (let i = 0; i < energyOutboxes.length; i++) {
			let onePair = [energyOutboxes[i], energyInbox, 'energy', 'source to upgrader '];
			logisticalPairs.push(onePair);
		}
	}
...
Room.prototype.enableCentralStorageLogic 					= function enableCentralStorageLogic() {
	this.memory.flags.centralStorageLogic = true;
	return true;
}
...
Room.prototype.toggleCentralStorageLogic 					= function toggleCentralStorageLogic() {
	const logicState = this.memory.flags.centralStorageLogic;
	if (logicState) {
		this.memory.flags.centralStorageLogic = false;
		return false;
	}
