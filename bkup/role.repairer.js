	...
	run: function (creep) {
		...
		if (!creep.memory.disableAI) {
			...
			if (creep.store.getUsedCapacity() == 0) {
				...
				switch (creep.room.memory.flags.centralStorageLogic) {
					case true: {
				...
				if (tower) {
					...
					if (Memory.rooms[creep.memory.homeRoom].flags.repairBasics) {
					...
					if (Memory.rooms[creep.memory.homeRoom].flags.repairRamparts) {
					...
					if (Memory.rooms[creep.memory.homeRoom].flags.repairWalls) {
