const roleUpgrader = {
		...
		if (creep.memory.canSeekEnergy === undefined) {
			if (creep.room.memory.flags.upgradersSeekEnergy !== undefined)
				creep.memory.canSeekEnergy = creep.room.memory.flags.upgradersSeekEnergy;
			else
				creep.memory.canSeekEnergy = true;
		}

		if (!creep.memory.mainBucket)
			//if (creep.room.memory.data.upgraderBucket)
			//	creep.memory.mainBucket = creep.room.memory.data.upgraderBucket;
			//else if (creep.room.controller.pos.findInRange(FIND_STRUCTURES, 5, { filter: { structureType: STRUCTURE_CONTAINER } }))
			//	creep.memory.mainBucket = creep.room.controller.pos.findClosestByRange(FIND_STRUCTURES, { filter: { structureType: STRUCTURE_CONTAINER } }).id;
			//else
		...
		if (!creep.memory.disableAI) {
			...
			if (creep.store.getUsedCapacity() == 0) { // I HAVE NO ENERGY, SO...
				...
				if (creep.memory.mainBucket && mainBucket !== null && mainBucket.store[RESOURCE_ENERGY] > 0) { // MY MAIN BUCKET IS HERE AND ISN'T EMPTY, SO...
